...
<div id="addServiceModal"
    ...
    <div
        ...
        <!-- Close Button -->
        <button id="closeAddServiceModal"
            class="absolute top-3 right-4 text-gray-400 hover:text-gray-600 text-[20px] font-bold">&times;</button>

        <!-- Header -->
        <div class="text-center mb-6">
            <h3 class="text-[16px] font-[Poppins] font-semibold text-gray-800">Add New Service</h3>
            <p class="text-[12px] font-[Roboto] text-gray-500 mt-1">Fill out the form below to register a new treatment service.</p>
        </div>
        ...
        <!-- Add Service Form -->
        <form action="{{ route('nurse-chief.submit-services') }}" method="POST">
            @csrf
            <div class="space-y-4">
                <!-- Service Name -->
                <div>
                    <label class="block text-[12px] font-[Poppins] text-gray-700 mb-1">Service Name</label>
                    <input type="text"
                        ...
                        placeholder="Enter service name" name="serviceName" required>
                </div>
                @error('serviceName')
                ...
                <!-- Category -->
                <div>
                    <label class="block text-[12px] font-[Poppins] text-gray-700 mb-1">Category</label>
                    <select
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-[12px] font-[Roboto] focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
                ...
                <input type="hidden" name="serviceAvailability" value="Available">
                <input type="hidden" name="agency_id" value="{{ auth()->user()->agency_id }}">
            </div>

            <!-- Action Buttons -->
            <div class="flex flex-col sm:flex-row justify-end gap-3 mt-6">
                <button type="button" id="cancelAddServiceModal"
...
<!-- Modal Script -->
<script>
    const openServiceModalBtn = document.getElementById('openAddServiceModal');
    const closeServiceModalBtn = document.getElementById('closeAddServiceModal');
    const cancelServiceModalBtn = document.getElementById('cancelAddServiceModal');
    const serviceModal = document.getElementById('addServiceModal');

    openServiceModalBtn.addEventListener('click', () => serviceModal.classList.remove('hidden'));
    closeServiceModalBtn.addEventListener('click', () => serviceModal.classList.add('hidden'));
    cancelServiceModalBtn.addEventListener('click', () => serviceModal.classList.add('hidden'));
