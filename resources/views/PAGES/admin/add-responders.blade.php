<x-layout.layout>
    <form class="max-w-3xl mx-auto bg-gray-100 shadow-lg rounded-2xl p-8 space-y-6"
        action="{{ route('bfp.submit-responders') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <!-- Choose Agency -->
            <div>
                <label class="block text-gray-700 mb-1 text-[12px] font-[Poppins]">Choose Agency</label>
                <select name="agency_id"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 text-[12px] font-[Poppins]
            ...
            <!-- Choose Role -->
            <div>
                <label class="block text-gray-700 mb-1 text-[12px] font-[Poppins]">Choose Role</label>
                <select name="user_type"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 text-[12px] font-[Poppins]
                   focus:ring-blue-500 focus:border-blue-500 bg-white">
                    <option value="">Select Role</option>
                    <option value="Operation Officer" {{ old('role') == 'Operation Officer' ? 'selected' : '' }}>
                        Operation Officer
                    </option>
                    <option value="Nurse Chief" {{ old('role') == 'Nurse Chief' ? 'selected' : '' }}>
                        Nurse Chief
                    </option>
                </select>
                @error('role')
        ...
        <div class="text-center">
            <h2 class="text-lg font-semibold text-gray-800 font-[Poppins]">Add Responders</h2>
