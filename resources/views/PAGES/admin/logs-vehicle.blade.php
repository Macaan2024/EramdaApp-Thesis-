<x-layout.layout>
    ...
    switch ($log->emergencyVehicle->vehicleTypes) {
    case 'Fire Truck': $fireTruckCount++; break;
    case 'Ambulance': $ambulanceCount++; break;
    case 'Police Car': $policeCarCount++; break;
    }
    }
    ...
    <!-- Stats Section -->
    <div class="flex flex-col lg:flex-row justify-between items-start gap-6 font-[Poppins]">
        <!-- Logs Stats -->
        <div class="w-full lg:w-1/2">
            <div class="flex flex-wrap gap-4 justify-start sm:justify-start">
                <!-- Add -->
                <div class="bg-red-600 text-white rounded-[10px] px-6 py-4 sm:px-8 sm:py-6 shadow-md hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-in-out w-[45%] sm:w-auto text-center">
                ...
                <!-- Edit -->
                <div class="bg-blue-600 text-white rounded-[10px] px-6 py-4 sm:px-8 sm:py-6 shadow-md hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-in-out w-[45%] sm:w-auto text-center">
                ...
                <!-- Delete -->
                <div class="bg-yellow-600 text-white rounded-[10px] px-6 py-4 sm:px-8 sm:py-6 shadow-md hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-in-out w-[45%] sm:w-auto text-center">
        ...
        <!-- Vehicles Stats -->
        <div class="w-full lg:w-1/2">
            <div class="flex flex-wrap gap-4 justify-end sm:justify-end">
                <!-- Fire Truck -->
                <div class="bg-red-600 text-white rounded-[10px] px-6 py-4 sm:px-8 sm:py-6 shadow-md hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-in-out w-[45%] sm:w-auto text-center">
                    <h3 class="text-[13px] sm:text-[14px] font-medium mb-1 opacity-90">Fire Trucks</h3>
                    <p class="text-[18px] sm:text-[20px] font-semibold">{{ $fireTruckCount }}</p>
                </div>

                <!-- Ambulance -->
                <div class="bg-blue-600 text-white rounded-[10px] px-6 py-4 sm:px-8 sm:py-6 shadow-md hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-in-out w-[45%] sm:w-auto text-center">
                    <h3 class="text-[13px] sm:text-[14px] font-medium mb-1 opacity-90">Ambulances</h3>
                    <p class="text-[18px] sm:text-[20px] font-semibold">{{ $ambulanceCount }}</p>
                </div>

                <!-- Police -->
                <div class="bg-yellow-600 text-white rounded-[10px] px-6 py-4 sm:px-8 sm:py-6 shadow-md hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-in-out w-[45%] sm:w-auto text-center">
                    <h3 class="text-[13px] sm:text-[14px] font-medium mb-1 opacity-90">Police Cars</h3>
        ...
        <form class="w-full md:max-w-md flex" action="" method="GET">
            <div class="relative w-full">
                ...
                <button type="submit"
                    ...
                    bg-blue-600 hover:bg-blue-700 text-white rounded-[10px] text-[12px] sm:text-[13px] font-medium px-4 py-2 transition">
                    Search
                </button>
            </div>
        </form>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="flex flex-wrap gap-3 text-[12px] sm:text-[13px] font-[Poppins]">
            <a href="/admin/logs-vehicles/All/{{ $id ?? '' }}"
                ...
                {{ $status === 'All' ? 'bg-gray-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-100' }}">
                All
            </a>
            <a href="/admin/logs-vehicles/Add/{{ $id ?? '' }}"
                class="px-5 py-2 rounded-[10px] border shadow-sm transition 
                {{ $status === 'Add' ? 'bg-blue-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-100' }}">
                Added
            </a>
            <a href="/admin/logs-vehicles/Update/{{ $id ?? '' }}"
                class="px-5 py-2 rounded-[10px] border shadow-sm transition 
                {{ $status === 'Update' ? 'bg-green-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-100' }}">
                Edited
            </a>
            <a href="/admin/logs-vehicles/Delete/{{ $id ?? '' }}"
                class="px-5 py-2 rounded-[10px] border shadow-sm transition 
                {{ $status === 'Delete' ? 'bg-red-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-100' }}">
                Deleted
            </a>
        </div>
    ...
    <!-- Logs Table -->
    <div class="overflow-x-auto shadow-lg rounded-[10px] border border-gray-200 bg-white mt-5 font-[Poppins]">
                ...
                <tr class="bg-white border-b hover:bg-blue-50 transition">
                    ...
                    <td class="px-4 sm:px-6 py-3">
                        ...
                        <img src="{{ asset('storage/' . $log->emergencyVehicle->vehicle_photo) }}"
                            alt="Vehicle Image" class="w-14 h-14 sm:w-16 sm:h-16 object-cover rounded-[10px] border">
                        @else
                    ...
                    <td class="px-4 sm:px-6 py-3">
                        <div class="flex flex-wrap gap-2">
                            @if ($log->interaction_type === 'Delete')
                            <button class="px-4 py-2 text-[12px] sm:text-[13px] font-medium rounded-[10px] bg-green-600 text-white shadow-sm hover:bg-green-700 hover:shadow-md active:scale-95 transition-all duration-200">
                                Restore
                            </button>
                            @else
                            <a href="{{ route('admin.logs-view-vehicles', $log->id) }}"
                                class="px-4 py-2 text-[12px] sm:text-[13px] font-medium rounded-[10px] bg-blue-600 text-white shadow-sm hover:bg-blue-700 hover:shadow-md active:scale-95 transition-all duration-200">
                                View
                            </a>
                            @endif
                        </div>
    ...
    <!-- Pagination -->
    <div class="mt-6 flex justify-center font-[Poppins] text-[12px] sm:text-[13px]">
        {{ $logs->links() }}
    </div>
