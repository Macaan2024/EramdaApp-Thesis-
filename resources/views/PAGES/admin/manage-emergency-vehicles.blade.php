<x-layout.layout>
        ...
        <form class="max-w-md w-full" action="{{ route('search-emergency-vehicles.admin') }}" method="GET">
            ...
            <div class="relative">
               ...
               focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Search Emergency Vehicles" name="search" value="{{ request('search') }}" required />
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-5">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @forelse ($emergencyVehicles as $emergencyVehicle)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 text-[12px] font-[Roboto] text-black">
                    <td class="px-6 py-4">{{ $emergencyVehicle->vehicleTypes }}</td>
                    <td class="px-6 py-4">{{ $emergencyVehicle->plateNumber }}</td>
                    <td class="px-6 py-4">{{ $emergencyVehicle->vehicle_photo }}</td>
                    <td class="px-6 py-4">{{ $emergencyVehicle->availabilityStatus }}</td>
                    <td class="px-6 py-4">
                        <div class="flex flex-row gap-1 items-center">
                            <a type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-[Poppins] rounded-lg text-[12px] px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 flex flex-row justify-center items-center gap-2" href="{{ route('edit-emergency-vehicles.admin', $emergencyVehicle->id) }}"><span class="material-symbols-outlined">
                                    eyeglasses_2
                                </span>View</a>
                            <a type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 rounded-lg  px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800 flex items-center justify-center" href="{{ route('edit-emergency-vehicles.admin', $emergencyVehicle->id) }}"><span class="material-symbols-outlined">
                                    edit
                                </span>Edit</a>
                            <form action="{{ route('delete-emergency-vehicles.admin', $emergencyVehicle->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
    ...
    <div class="mt-10 flex justify-center">
        {{ $emergencyVehicles->appends(request()->query())->links('vendor.pagination.tailwind') }}
    </div>
    <x-partials.stack-js />
</x-layout.layout>